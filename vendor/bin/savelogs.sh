...
export_general_log() {
	...
	############################################################################################
	# save fscklog
	cat /dev/fscklogs/logf2fs > $GENERAL_LOG/fscklog.txt
	echo "cat /dev/fscklogs/logf2fs > $GENERAL_LOG/fscklog.txt"
	############################################################################################
	# save network info
...
#	mount -t vfat /dev/block/bootdevice/by-name/logfs /vendor/logfs
	...
	cp -r /data/logcat_log/tcpdump* $GENERAL_LOG/logcat_log
	...
	cp /asdf/logcat-charger.txt $GENERAL_LOG/charger_debug/
	############################################################################################
...
#	done
	...
	############################################################################################
	#mv /data/vendor/ramdump/ssr_ramdump/ $SAVE_LOG_PATH
	cp -r  /data/vendor/ramdump/ssr_ramdump/ $GENERAL_LOG
	#rm -r  /data/vendor/ramdump/ssr_ramdump/
	echo "mv /data/vendor/ramdump/ssr_ramdump $GENERAL_LOG"
...
export_tcpdump_log() {
	...
	cp -r /data/logcat_log/tcpdump* $ROOT_PATH/tcpdump
...
save_general_log() {
	...
	############################################################################################
	# save fscklog
	cat /dev/fscklogs/logf2fs > $SAVE_LOG_PATH/fscklog.txt
	echo "cat /dev/fscklogs/logf2fs > $SAVE_LOG_PATH/fscklog.txt"
	############################################################################################
	# save network info
	cat /proc/net/route > $GENERAL_LOG/route.txt
	echo "cat /proc/net/route > $GENERAL_LOG/route.txt"
...
save_tcpdump_log() {
	if [ -d "/data/logcat_log" ]; then
		if [ ".$is_tcpdump_status" == ".running" ]; then
			...
			start tcpdump-warp
			for fname in /data/logcat_log/tcpdump.pcap*
			do
				if [ -e $fname ]; then
					if [ ".$fname" != "./data/logcat_log/tcpdump.pcap0" ]; then
